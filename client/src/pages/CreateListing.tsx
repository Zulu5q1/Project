...
import type { University, Campus, Category } from "../types";
...
export default function CreateListing() {
  ...
  const [universityId, setUniversityId] = useState(user?.university?.id || "");
  const [campusId, setCampusId] = useState(user?.campus?.id || "");
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-6">
        <div>
          ...
          <input id="title" type="text" value={title} onChange={(e) => setTitle(e.target.value)}
            className={inputClass} placeholder="What are you selling?" disabled={loading} maxLength={200} />
        </div>

        <div>
          <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Description *</label>
          <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)}
            className={`${inputClass} h-32 resize-y`} placeholder="Describe your item in detail..." disabled={loading} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input id="price" type="number" min="0" step="100" value={price} onChange={(e) => setPrice(e.target.value)}
              className={inputClass} placeholder="0" disabled={loading} />
          </div>
          <div>
        ...
        <div>
          ...
          <select id="category" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}
            className={inputClass} disabled={loading || loadingCategories}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select id="university" value={universityId} onChange={(e) => { setUniversityId(e.target.value); setCampusId(""); }}
              className={inputClass} disabled={loading || loadingUniversities}>
              <option value="">{loadingUniversities ? "Loading..." : "Select a university"}</option>
          ...
          <div>
            ...
            <select id="campus" value={campusId} onChange={(e) => setCampusId(e.target.value)}
              className={inputClass} disabled={loading || !universityId || loadingCampuses}>
              <option value="">
                {loadingCampuses ? "Loading..." : !universityId ? "Select university first" : "Select a campus"}
              </option>
              {campuses.map((c) => (
        ...
        <div>
          ...
          <input id="type" type="text" value={location} onChange={(e) => setLocation(e.target.value)}
            className={inputClass} placeholder="e.g. Near main gate, Akoka" disabled={loading} />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Images (optional)</label>
          <p className="text-xs text-gray-400 mb-3">Provide image URLs. In development, paste direct links (e.g. from imgur).</p>
          {imageUrls.map((url, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <input type="url" value={url} onChange={(e) => updateImageUrl(i, e.target.value)}
                className={`${inputClass} flex-1`} placeholder="https://example.com/image.jpg" disabled={loading} />
              {imageUrls.length > 1 && (
